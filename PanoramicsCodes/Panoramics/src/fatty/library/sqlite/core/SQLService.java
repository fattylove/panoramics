...
public class SQLService {
	...
	private static HashMap<String, SQLService> daoMap = new HashMap<String, SQLService>();
	 ...
	 * 初始化SQLService
	 * 
	 * @param config DaoConfig-dao层配置
	 */
	private SQLService(DaoConfig config) {
		if (config == null)
	 ...
	 * 初始化 - 同步单例创建
	 * 
	 * @param daoConfig  DaoConfig-dao层配置
	 * @return
	 */
	private synchronized static SQLService getInstance(DaoConfig daoConfig) {
		SQLService dao = daoMap.get(daoConfig.getDbName());
		if (dao == null) {
			dao = new SQLService(daoConfig);
	 ...
	 * @param context
	 */
	public static SQLService create(Context context) {
	 ...
	 */
	public static SQLService create(Context context, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 * 
	 */
	public static SQLService create(Context context, String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 */
	public static SQLService create(Context context, String dbName, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setTargetDirectory(android.os.Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + ".panoramics");
		config.setDbName(dbName);
		config.setDebug(isDebug);
		return create(config);
	}
	 ...
	 * 
	 */
	public static SQLService create(Context context, String targetDirectory, String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
	 ...
	 *            是否为debug模式（debug模式进行数据库操作的时候将会打印sql语句）
	 */
	public static SQLService create(Context context, String targetDirectory, String dbName, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setTargetDirectory(targetDirectory);
		config.setDbName(dbName);
	 ...
	 * @return
	 */
	public static SQLService create(Context context, String dbName, boolean isDebug, int dbVersion, DbUpdateListener dbUpdateListener) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
		config.setDebug(isDebug);
	 ...
	 * @return
	 */
	public static SQLService create(Context context, String targetDirectory, String dbName, boolean isDebug, int dbVersion, DbUpdateListener dbUpdateListener) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setTargetDirectory(targetDirectory);
		config.setDbName(dbName);
	...
	public boolean saveBindId(Object entity) {
		checkTableExist(entity.getClass());
		List<TABLE_KeyValue> entityKvList = SQLBuilder.getSaveKeyValueListByEntity(entity);
		if (entityKvList != null && entityKvList.size() > 0) {
			TABLE_TableEntity tf = TABLE_TableEntity.get(entity.getClass());
	 ...
	 * @param cv
	 */
	private void insertContentValues(List<TABLE_KeyValue> list, ContentValues cv) {
		if (list != null && cv != null) {
			for (TABLE_KeyValue kv : list) {
	 ...
	 * @param sqlInfo
	 */
	private void exeSqlInfo(SQLStatement sqlInfo) {
		if (sqlInfo != null) {
	...
	public <T> T findById(Object id, Class<T> clazz) {
		checkTableExist(clazz);
		SQLStatement sqlInfo = SQLBuilder.getSelectSqlAsSqlInfo(clazz, id);
		if (sqlInfo != null) {
			debugSql(sqlInfo.getSql());
			Cursor cursor = db.rawQuery(sqlInfo.getSql(), sqlInfo.getBindArgsAsStringArray());
			try {
				if (cursor.moveToNext()) {
					return TOOL_Cursor.getEntity(cursor, clazz, this);
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
	...
	public <T> T findWithManyToOneById(Object id, Class<T> clazz) {
		...
		debugSql(sql);
		SQLColumn dbModel = findDbModelBySQL(sql);
		if (dbModel != null) {
			T entity = TOOL_Cursor.dbModel2Entity(dbModel, clazz);
	 ...
	 * @return
	 */
	public <T> T loadManyToOne(SQLColumn dbModel, T entity, Class<T> clazz, Class<?>... findClass) {
		if (entity != null) {
			try {
				Collection<TABLE_MTO> manys = TABLE_TableEntity.get(clazz).manyToOneMap.values();
				for (TABLE_MTO many : manys) {
					Object id = null;
					if (dbModel != null) {
						id = dbModel.get(many.getColumn());
					} else if (many.getValue(entity).getClass() == MTOLazyLoader.class && many.getValue(entity) != null) {
						id = ((MTOLazyLoader) many.getValue(entity)).getFieldValue();
					}
					if (id != null) {
						...
						if (isFind) {
							...
							if (manyEntity != null) {
								if (many.getValue(entity).getClass() == MTOLazyLoader.class) {
									if (many.getValue(entity) == null) {
										many.setValue(entity, new MTOLazyLoader(entity, clazz, many.getManyClass(), this));
									}
									((MTOLazyLoader) many.getValue(entity)).set(manyEntity);
								} else {
	...
	public <T> T loadOneToMany(T entity, Class<T> clazz, Class<?>... findClass) {
		if (entity != null) {
			try {
				Collection<TABLE_OTM> ones = TABLE_TableEntity.get(clazz).oneToManyMap.values();
				Object id = TABLE_TableEntity.get(clazz).getId().getValue(entity);
				for (TABLE_OTM one : ones) {
					boolean isFind = false;
					if (findClass == null || findClass.length == 0) {
						isFind = true;
					}
					...
					if (isFind) {
						...
						if (list != null) {
							...
							if (one.getDataType() == OTMLazyLoader.class) {
								OTMLazyLoader oneToManyLazyLoader = one.getValue(entity);
	...
	public <T> List<T> findAllBySql(Class<T> clazz, String strSQL) {
		...
		try {
			...
			while (cursor.moveToNext()) {
				T t = TOOL_Cursor.getEntity(cursor, clazz, this);
				list.add(t);
			}
			return list;
		} catch (Exception e) {
	 ...
	 * @param strSQL
	 */
	public SQLColumn findDbModelBySQL(String strSQL) {
		debugSql(strSQL);
		Cursor cursor = db.rawQuery(strSQL, null);
		try {
			if (cursor.moveToNext()) {
				return TOOL_Cursor.getDbModel(cursor);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	 ...
	 * @return
	 */
	public List<SQLColumn> findDbModelListBySQL(String strSQL) {
		debugSql(strSQL);
		Cursor cursor = db.rawQuery(strSQL, null);
		List<SQLColumn> dbModelList = new ArrayList<SQLColumn>();
		try {
			while (cursor.moveToNext()) {
				dbModelList.add(TOOL_Cursor.getDbModel(cursor));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void checkTableExist(Class<?> clazz) {
		if (!tableIsExist(TABLE_TableEntity.get(clazz))) {
	 ...
	 * @return
	 */
	private boolean tableIsExist(TABLE_TableEntity table) {
	...
	public static class DaoConfig {
		...
		private String mDbName = "panoramics.db";
		private int dbVersion = 1; // 数据库版本
		private boolean debug = true; // 是否是调试模式
		private DbUpdateListener dbUpdateListener;
		private String targetDirectory;// sdcard文件目录
	...
	private SQLiteDatabase createDbFileOnSDCard(String sdcardPath,
		...
		if (!dbf.exists()) {
			try {
				...
				throw new SQLException("数据库文件创建失败", ioex);
			}
		} else {
			return SQLiteDatabase.openOrCreateDatabase(dbf, null);
		}
