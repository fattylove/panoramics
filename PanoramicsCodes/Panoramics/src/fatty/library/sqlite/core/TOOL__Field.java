...
public class TOOL__Field {
	...
	public static Field getFieldByColumnName(Class<?> clazz, String columnName) {
		...
		if (columnName != null) {
			...
			if (fields != null && fields.length > 0) {
				if (columnName.equals(TOOL_Class.getPrimaryKeyColumn(clazz)))
					field = TOOL_Class.getPrimaryKeyField(clazz);

				if (field == null) {
					for (Field f : fields) {
						ANNOTATION_PROPERTY property = f.getAnnotation(ANNOTATION_PROPERTY.class);
						if (property != null&& columnName.equals(property.column())) {
							field = f;
							break;
						}
						ANNOTATION_MTO manyToOne = f.getAnnotation(ANNOTATION_MTO.class);
	...
	public static String getColumnByField(Field field) {
		ANNOTATION_PROPERTY property = field.getAnnotation(ANNOTATION_PROPERTY.class);
		if (property != null && property.column().trim().length() != 0) {
			return property.column();
		}

		ANNOTATION_MTO manyToOne = field.getAnnotation(ANNOTATION_MTO.class);
		if (manyToOne != null && manyToOne.column().trim().length() != 0) {
			return manyToOne.column();
		}

		ANNOTATION_OTM oneToMany = field.getAnnotation(ANNOTATION_OTM.class);
		if (oneToMany != null && oneToMany.manyColumn() != null
				&& oneToMany.manyColumn().trim().length() != 0) {
			return oneToMany.manyColumn();
		}

		ANNOTATION_ID id = field.getAnnotation(ANNOTATION_ID.class);
	...
	public static boolean isTransient(Field f) {
		return f.getAnnotation(ANNOTATION_TRANSIENT.class) != null;
	}
	...
	public static boolean isManyToOne(Field field) {
		return field.getAnnotation(ANNOTATION_MTO.class) != null;
	}

	public static boolean isOneToMany(Field field) {
		return field.getAnnotation(ANNOTATION_OTM.class) != null;
	}
