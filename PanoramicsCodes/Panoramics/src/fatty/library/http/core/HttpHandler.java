...
public class HttpHandler<T> extends A_AsyncTask<Object, Object, Object> implements EntityCallBack {
	...
	public HttpHandler(AbstractHttpClient client, HttpContext context, CallBack<T> callback, String charset) {
		this.client = client;
	...
	protected void onProgressUpdate(Object... values) {
		...
		case UPDATE_SUCCESS:
			if (callback != null)
				callback.onSuccess((T) values[1] , (Integer) values[2]);
			break;
		default:
			break;
		}
	...
	private void handleResponse(HttpResponse response) {
		...
		if (status.getStatusCode() >= 300) {
			...
			try {
				...
				publishProgress(UPDATE_SUCCESS, responseBody ,status.getStatusCode());
			} catch (IOException e) {
